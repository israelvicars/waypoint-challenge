...
export function readIEP(studentId: string): string {
  ...
  if (!rel) throw new Error(`Unknown student ID: "${studentId}". Known IDs: ${Object.keys(STUDENT_FILES).join(', ')}`);
...
export function readLesson(lessonId: string): string {
  ...
  if (!rel) throw new Error(`Unknown lesson ID: "${lessonId}". Known IDs: ${Object.keys(LESSON_FILES).join(', ')}`);
  return readFileSync(path.join(dataDir(), rel), 'utf-8');
}
