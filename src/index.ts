...
server.tool(
  'get_student_profile',
  'Returns the Tier 1 Key Profile Summary for a student — the primary context for modification generation.',
  { student_id: z.string().describe('Student ID, e.g. "jasmine-bailey" or "marcus-chen"') },
  async ({ student_id }) => {
    try {
      return { content: [{ type: 'text' as const, text: getStudentProfile(student_id) }] };
    } catch (err) {
...
server.tool(
  'get_iep_goal',
  'Returns a specific IEP goal record (Tier 2) for a student. Use when tagging a recommendation to a specific goal or citing exact target language.',
  {
  ...
  async ({ student_id, goal_number }) => {
    try {
      return { content: [{ type: 'text' as const, text: getIepGoal(student_id, goal_number) }] };
    } catch (err) {
      return { content: [{ type: 'text' as const, text: String(err) }], isError: true };
    }
  }
...
server.tool(
  'get_accommodations',
  'Returns the flat accommodation list (Tier 3) for a student. Useful for generating the before-class checklist.',
  { student_id: z.string().describe('Student ID') },
  async ({ student_id }) => {
    try {
      return { content: [{ type: 'text' as const, text: getAccommodations(student_id) }] };
    } catch (err) {
      return { content: [{ type: 'text' as const, text: String(err) }], isError: true };
    }
  }
...
server.tool(
  'get_lesson',
  'Returns the full processed lesson document with verbatim activity text, question labels (DRQ-, MC-, SR-, DISC-), and vocabulary.',
  { lesson_id: z.string().describe('Lesson ID, e.g. "what-is-community"') },
  async ({ lesson_id }) => {
    try {
      return { content: [{ type: 'text' as const, text: getLesson(lesson_id) }] };
    } catch (err) {
      return { content: [{ type: 'text' as const, text: String(err) }], isError: true };
    }
  }
...
server.tool(
  ...
  'Generates a complete IEP-grounded modification guide for a lesson and one or more students. ' +
...
server.tool(
  'ingest_iep',
  'Ingests an IEP PDF, extracts structured markdown using claude-opus-4-7, and writes it to data/ieps/. ' +
...
server.tool(
  'ingest_lesson',
  'Ingests a lesson plan PDF, extracts structured markdown using claude-opus-4-7, and writes it to data/lessons/. ' +
