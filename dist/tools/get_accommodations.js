...
export function getAccommodations(studentId) {
    const markdown = readIEP(studentId);
    const acc = extractAccommodations(markdown);
    if (!acc)
        throw new Error(`No Accommodations section found in IEP for student "${studentId}"`);
    return acc;
}
//# sourceMappingURL=get_accommodations.js.map