...
**Scenario 1 — Single student** (Jasmine Bailey × What is Community):  
→ [`examples/scenario_1_jasmine.md`](./examples/scenario_1_jasmine.md)  
Student-organized guide with before-class checklist, four activity sections, IEP goal and accommodation tags on every recommendation, and copy-paste-ready scaffolds derived from the actual lesson questions.

**Scenario 2 — Multi-student** (Jasmine Bailey + Marcus Chen × What is Community):  
→ [`examples/scenario_2_jasmine_marcus.md`](./examples/scenario_2_jasmine_marcus.md)  
Activity-organized guide with per-student modifications at each lesson step, plus a synthesis section covering overlapping needs, divergent needs, and instructional opportunities created by this specific student combination.

Both outputs were generated by `claude-opus-4-7` using the `generate_modifications` tool against pre-ingested IEP and lesson markdown. No post-editing.
